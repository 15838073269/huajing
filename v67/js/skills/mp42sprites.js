/**
 * 视频转序列帧技能
 * 基于 v63/js/mp42sprites.js 改写为 v67 插件
 *
 * 功能：
 *   - 视频文件上传
 *   - 按帧提取（10fps）
...
var Mp42SpritesSkill = {
    ...
    /**
     * 从画布选中视频自动导入
     * 流程：先在画布点选视频 → 再点插件按钮 → 视频自动载入抽帧
     */
    _tryLoadFromCanvas: function() {
        if (typeof CanvasImages === 'undefined') return;
        var video = CanvasImages.getSelectedVideo();
        if (video) {
            this._loadVideoFromUrl(video.url, video.name);
            if (typeof showToast === 'function') showToast('已从画布导入视频：' + (video.name || ''));
        } else {
            // 检查是否选中了图片
            var imgDataURL = CanvasImages.getSelected('display');
            if (imgDataURL) {
                if (typeof showToast === 'function') showToast('当前选中的是图片，不是视频');
            } else {
                if (typeof showToast === 'function') showToast('请先在画布中点选一个视频，再激活帧插件');
            }
        }
    },

    // ===== 画布选中实时联动（窗口常驻时，点选别的画布【视频】自动切换抽帧源；选中图片则忽略） =====
    ...
    _onCanvasFollow: function() {
        if (typeof CanvasImages === 'undefined') return;
        var video = CanvasImages.getSelectedVideo();
        if (video) this._loadVideoFromUrl(video.url, video.name);
        // 选中图片时不处理（避免误弹提示、也不切换源）
    },

    /**
     * 从 URL 加载视频并抽帧（复用 _handleVideoUpload 的核心逻辑）
     */
    _loadVideoFromUrl: function(url, name) {
        var self = this;
    ...
    _renderTabBar: function() {
        ...
        html += '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msExportAllBtn" style="flex-shrink:0;margin-left:auto;">GoDot总导出</button>';
        bar.innerHTML = html;
    ...
    _createOverlay: function() {
        ...
        // 恢复保存的尺寸/位置（使用公共库逻辑）
        var savedW = 1200, savedH = 700, savedL = null, savedT = null;
        try {
            var saved = JSON.parse(localStorage.getItem('ms-window-rect'));
            if (saved) {
                var sw = window.innerWidth, sh = window.innerHeight;
                savedW = Math.min(saved.w || 1200, sw - 20);
                savedH = Math.min(saved.h || 700, sh - 20);
        ...
        ov.style.left = (savedL !== null ? savedL : Math.max(20, (window.innerWidth - savedW) / 2)) + 'px';
        ov.style.top = (savedT !== null ? savedT : Math.max(20, (window.innerHeight - savedH) / 2)) + 'px';
        ov.style.minWidth = '800px';
        ov.style.minHeight = '500px';
        ...
        header.innerHTML =
            '<span class="cos-pwin-hdr-title">帧动画</span>' +
            '<div class="cos-pwin-hdr-right">' +
                '<span class="cos-pclose" id="msCloseBtn" title="关闭">\u00d7</span>' +
            '</div>';
        ...
        leftTop.innerHTML =
            '<div class="ms-cell-header">原图帧播放' +
                '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msPlayOrigBtn" disabled>播放</button>' +
            '</div>' +
        ...
        rightTop.innerHTML =
            ...
            '<div class="ms-upload-area">' +
                '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
                    '<button class="cos-pbtn cos-pbtn-danger cos-pbtn-sm" id="msGridVideoBtn">多人网格分割</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msSlicerOpenBtn">单张多动作序列图</button>' +
                '</div>' +
            '</div>' +
            '<div class="ms-frames-info" id="msFramesInfo">在画布中点选视频后激活本插件，自动抽帧</div>' +
            ...
            '<div class="ms-selected-section" id="msSelectedSection">' +
                '<div class="ms-section-header">' +
                    '<div class="ms-hdr-left">' +
                        ...
                        '<span class="cos-pbadge" id="msSelectedBadge">0 帧</span>' +
                    '</div>' +
                    '<div class="ms-hdr-controls">' +
                        '<span class="ms-hdr-label">缩放</span>' +
                        ...
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msSaveCanvasBtn" disabled>保存到画布</button>' +
                        '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msDownloadBtn" disabled>下载</button>' +
                        '<span style="display:inline-flex;align-items:center;gap:3px;margin:0 2px;">' +
                            '<span class="cos-plabel">动画名</span>' +
                            '<input type="text" id="msAnimName" value="default" class="cos-pinput" style="width:80px;">' +
                        '</span>' +
                        '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msGodotBtn" disabled>Godot导出</button>' +
                        '<button class="cos-pbtn cos-pbtn-warn cos-pbtn-sm" id="msClearBtn" disabled>清空</button>' +
                    '</div>' +
                '</div>' +
                '<div class="ms-select-toolbar" id="msSelectToolbar">' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msSelAllBtn">全选</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msOddBtn">奇数</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msEvenBtn">偶数</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msHalfBtn">二分</button>' +
                    '<span class="cos-plabel">循环</span>' +
                    '<input type="number" class="cos-pinput" id="msLoopInput" value="2" min="2" style="width:48px;text-align:center;">' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" id="msLoopBtn">提取</button>' +
                '</div>' +
        ...
        leftBottom.innerHTML =
            '<div class="ms-cell-header">已选帧播放' +
                '<div style="display:flex;gap:4px;align-items:center;">' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msPrevBtn" disabled>⏮</button>' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msPlayPauseBtn" disabled>▶</button>' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msNextBtn" disabled>⏭</button>' +
                    '<label class="cos-plabel" style="margin-left:8px;display:flex;align-items:center;gap:3px;cursor:pointer;">' +
            ...
            '<div class="ms-cell-body" style="position:relative;">' +
                ...
                '<div id="msFrameIndicator" style="position:absolute;bottom:8px;left:50%;transform:translateX(-50%);' +
                    'background:rgba(0,0,0,0.7);color:var(--cos-accent);padding:4px 12px;border-radius:6px;font-size:12px;font-weight:700;' +
        ...
        loadingOverlay.innerHTML =
            ...
            '</div>';
        ov.appendChild(loadingOverlay);

        document.body.appendChild(ov);

        // 缩放（使用公共 WindowHelper）
        if (typeof WindowHelper !== 'undefined') {
            WindowHelper.makeResizable(ov, { minWidth: 800, minHeight: 500, storeKey: 'ms-window-rect' });
        }

        // 拖拽（使用公共 CosUI.draggable）
        if (typeof CosUI !== 'undefined' && CosUI.draggable) {
            CosUI.draggable.bind(ov, header, {
                storeKey: 'ms-window-rect',
                closeSelector: '#msCloseBtn,.cos-pclose'
            });
        }
    ...
    _bindEvents: function() {
        ...
        // 保存到画布
        ov.querySelector('#msSaveCanvasBtn').addEventListener('click', function() {
    ...
    _handleVideoUpload: function(e) {
        ...
        if (!file) return;
        this._cfLock = true; // 手动导入视频后暂停画布跟随以免覆盖
    ...
    _saveToCanvas: function() {
        ...
        var animName = (this._overlay.querySelector('#msAnimName') || {}).value || '帧序列图';
        var label = animName + ' ' + new Date().toLocaleTimeString();

        // 获取源视频 ID 作为 parentId，使结果出现在原图旁边
        var parentId = null;
        var video = CanvasImages.getSelectedVideo();
        if (video) parentId = video.id;

        CanvasImages.place(stripDataURL, null, null, label, parentId).then(function() {
            if (typeof showToast === 'function') showToast('已保存到画布：' + label);
        }).catch(function(err) {
            console.error('[mp42sprites] saveToCanvas error:', err);
    ...
    _updateButtons: function() {
        ...
        var saveCanvasBtn = ov.querySelector('#msSaveCanvasBtn');
        if (saveCanvasBtn) saveCanvasBtn.disabled = !has;
    ...
    _createSlicerUI: function() {
        ...
        panel.innerHTML =
            '<div class="ms-slicer-header">' +
                ...
                '<span class="cos-pclose" id="msSlClose" title="关闭">\u00d7</span>' +
            '</div>' +
            '<div class="ms-slicer-body">' +
                ...
                '<div class="ms-slicer-right">' +
                    ...
                    '<div style="display:flex;gap:8px;margin-top:auto;padding-top:10px;">' +
                        '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msSlImportBtn" style="flex:1;" disabled>导入到列表</button>' +
                        '<button class="cos-pbtn cos-pbtn-warn cos-pbtn-sm" id="msSlCancelBtn">取消</button>' +
                    '</div>' +
                '</div>' +
            '</div>';
    ...
    _createGridSplitterUI: function() {
        ...
        panel.innerHTML =
            '<div class="ms-grids-header">' +
                ...
                '<span class="cos-pclose" id="msGsClose" title="关闭">\u00d7</span>' +
            '</div>' +
            '<div class="ms-grids-body">' +
                '<div class="ms-grids-left">' +
                    ...
                    '<div class="ms-grids-controls">' +
                        '<div style="display:flex;align-items:center;gap:6px;">' +
                            '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" id="msGsPlayBtn" disabled>\u25b6</button>' +
                    ...
                    '<div class="ms-grids-config">' +
                        ...
                        '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msGsExtractBtn" disabled>提取序列帧</button>' +
                    '</div>' +
                ...
                '<div class="ms-grids-right">' +
                    '<div style="display:flex;justify-content:space-between;align-items:center;flex-shrink:0;">' +
                        ...
                        '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" id="msGsExportBtn" disabled>导出选中</button>' +
                        '<button class="cos-pbtn cos-pbtn-warn cos-pbtn-sm" id="msGsMergeBtn" disabled>合成大图</button>' +
                    '</div>' +
...
/* ===== 内联样式（仅插件专属组件，公共样式由 css/plugin-theme.css 提供） ===== */
(function() {
    var s = document.createElement('style');
    s.textContent =
        /* 标签栏 */
        '.ms-tab-bar { display:flex;align-items:center;gap:2px;padding:4px 8px;background:var(--cos-surface);' +
            'border-bottom:1px solid var(--cos-border);flex-shrink:0;overflow-x:auto; }' +
        '.ms-tab { padding:4px 12px;font-size:11px;border-radius:5px;background:var(--cos-surface-warm);' +
            'color:var(--cos-text-soft);cursor:pointer;white-space:nowrap;display:flex;align-items:center;gap:4px;position:relative; }' +
        '.ms-tab:hover { background:rgba(255,255,255,0.08);color:var(--cos-text); }' +
        '.ms-tab-active { background:var(--cos-accent-soft);color:var(--cos-accent); }' +
        '.ms-tab-close { font-size:12px;color:var(--cos-text-dim);padding:2px 6px;border-radius:3px;margin-left:6px; }' +
        '.ms-tab-close:hover { color:#ef4444;background:rgba(239,68,68,0.15); }' +
        '.ms-tab-add { background:transparent;border:1px dashed var(--cos-border);color:var(--cos-text-dim); }' +
        '.ms-tab-add:hover { border-color:var(--cos-accent);color:var(--cos-accent); }' +

        /* 主体：2列2行 grid */
        '.ms-body { display:grid;grid-template-columns:280px 1fr;grid-template-rows:1fr 1fr; }' +
        '.ms-grid-cell { overflow-y:auto;padding:12px;display:flex;flex-direction:column; }' +
        '.ms-left-top { grid-column:1;grid-row:1;background:var(--cos-surface-warm);border-right:1px solid var(--cos-border);' +
            'border-bottom:1px solid var(--cos-border); }' +
        '.ms-left-bottom { grid-column:1;grid-row:2;background:var(--cos-surface-warm);border-right:1px solid var(--cos-border); }' +
        '.ms-right-top { grid-column:2;grid-row:1;background:rgba(255,255,255,.02);border-bottom:1px solid var(--cos-border);gap:6px; }' +
        '.ms-right-bottom { grid-column:2;grid-row:2;background:rgba(255,255,255,.02);gap:6px; }' +

        /* 左列播放器 */
        '.ms-cell-header { font-size:11px;font-weight:700;color:var(--cos-text-soft);letter-spacing:1px;' +
            'display:flex;align-items:center;justify-content:space-between;flex-shrink:0;padding-bottom:8px;' +
            'border-bottom:1px solid var(--cos-border);margin-bottom:8px; }' +
        '.ms-cell-header::before { content:"";width:3px;height:12px;background:var(--cos-accent);border-radius:2px;' +
            'margin-right:8px; }' +
        ...
        '.ms-progress-fill { height:100%;background:linear-gradient(90deg,var(--cos-accent),#7dd3fc);width:0%;' +
        ...
        /* 帧信息 */
        '.ms-frames-info { padding:8px 12px;font-size:12px;color:var(--cos-text-soft);background:rgba(0,0,0,.2);border-radius:8px;' +
            'margin-bottom:14px;border:1px solid var(--cos-border); }' +

        /* 区块 */
        '.ms-frames-section,.ms-selected-section { margin-bottom:16px;' +
            'padding:14px;background:rgba(255,255,255,.02);border-radius:10px;border:1px dashed var(--cos-border); }' +
        ...
        '.ms-section-title { font-size:13px;color:var(--cos-text);font-weight:700; }' +
        ...
        '.ms-hdr-label { font-size:11px;color:var(--cos-text-dim);white-space:nowrap; }' +
        '.ms-hdr-slider { width:80px;height:3px;-webkit-appearance:none;appearance:none;' +
            'background:var(--cos-accent-soft);border-radius:2px;outline:none;cursor:pointer; }' +
        '.ms-hdr-slider::-webkit-slider-thumb { -webkit-appearance:none;appearance:none;' +
            'width:10px;height:10px;border-radius:50%;background:var(--cos-accent);border:none;cursor:pointer; }' +
        '.ms-hdr-val { font-size:10px;color:var(--cos-text-soft);min-width:20px;text-align:center; }' +

        /* 帧网格 — 横向滚动 */
        '.ms-frames-grid { display:flex;gap:8px;padding:10px;background:rgba(0,0,0,.2);' +
            'border-radius:10px;border:1px solid var(--cos-border);overflow-x:auto;overflow-y:hidden;min-height:170px;flex-shrink:0; }' +
        ...
        '.ms-frame-item.selected { border-color:var(--cos-accent);box-shadow:0 0 12px var(--cos-accent-soft); }' +
        ...
        '.ms-frame-name { position:absolute;bottom:0;left:0;right:0;background:rgba(0,0,0,.65);color:var(--cos-text-soft);' +
            'padding:2px 4px;font-size:9px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center; }' +
        '.ms-play-order { position:absolute;top:3px;right:26px;background:var(--cos-accent);color:#fff;' +
        ...
        '.ms-frame-check { position:absolute;top:3px;right:3px;width:20px;height:20px;background:var(--cos-accent);' +
        ...
        '.ms-selected-grid { display:flex;flex-wrap:wrap;gap:6px;min-height:50px;padding:8px;background:rgba(0,0,0,.2);' +
            'border-radius:8px;border:1px dashed var(--cos-border);flex:1;transition:all .15s; }' +
        ...
        '.ms-selected-grid.drag-over { border-color:var(--cos-accent);background:var(--cos-accent-soft); }' +
        ...
        '.ms-selected-item.dragging { opacity:.5;transform:scale(1.1);box-shadow:0 6px 20px var(--cos-accent-soft); }' +
        '.ms-selected-item.drag-over { border:2px dashed var(--cos-accent); }' +
        '.ms-selected-item:hover { transform:scale(1.08);box-shadow:0 4px 12px var(--cos-accent-soft); }' +
        ...
        '.ms-selected-item .ms-selected-name { font-size:10px;color:var(--cos-text-soft);text-align:center;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding:1px 2px; }' +
        '.ms-selected-order { position:absolute;top:2px;left:2px;background:var(--cos-accent);color:#fff;' +
        ...
        '.ms-loading-content { background:var(--cos-surface-warm);padding:30px 40px;border-radius:14px;text-align:center;border:1px solid var(--cos-border); }' +
        '.ms-spinner { width:36px;height:36px;border:3px solid var(--cos-accent-soft);border-top:3px solid var(--cos-accent);' +
        ...
        '.ms-loading-text { font-size:13px;color:var(--cos-text-soft); }' +
        ...
        '.ms-grid-cell::-webkit-scrollbar-thumb, .ms-frames-grid::-webkit-scrollbar-thumb, .ms-selected-grid-single::-webkit-scrollbar-thumb { background:var(--cos-accent-soft);border-radius:3px; }' +
        ...
        '.ms-slicer-panel { width:920px;height:680px;background:var(--cos-world);border-radius:12px;' +
        ...
        '.ms-slicer-header { display:flex;justify-content:space-between;align-items:center;' +
            'padding:10px 20px;background:var(--cos-surface-warm);border-bottom:1px solid var(--cos-border);flex-shrink:0; }' +
        ...
        '.ms-slicer-right { width:270px;padding:16px;display:flex;flex-direction:column;gap:10px;' +
            'border-left:1px solid var(--cos-border);overflow-y:auto;flex-shrink:0; }' +
        '.ms-slicer-info { font-size:11px;color:var(--cos-text-soft);padding:6px 10px;background:rgba(0,0,0,.2);' +
            'border-radius:6px;border:1px solid var(--cos-border);line-height:1.6; }' +
        '.ms-slicer-info span { color:var(--cos-accent);font-weight:700; }' +
        ...
        '.ms-slicer-grid-input { width:60px;padding:4px 8px;background:rgba(0,0,0,.3);border:1px solid var(--cos-border);' +
            'border-radius:6px;color:var(--cos-text);font-size:12px;text-align:center;outline:none; }' +
        ...
        '.ms-grids-overlay { position:fixed;left:0;top:0;width:100%;height:100%;' +
            'background:transparent;display:none;align-items:center;justify-content:center;pointer-events:none; }' +
        '.ms-grids-panel { width:1100px;height:740px;background:var(--cos-world);border-radius:12px;' +
            'display:flex;flex-direction:column;overflow:hidden;box-shadow:0 8px 40px rgba(0,0,0,.6);pointer-events:auto; }' +
        ...
        '.ms-grids-input { width:40px;padding:3px 6px;background:rgba(0,0,0,.3);border:1px solid var(--cos-border);' +
            'border-radius:6px;color:var(--cos-text);font-size:12px;text-align:center;outline:none; }' +
        '.ms-grids-input:focus { border-color:rgba(239,68,68,.4); }' +
        '.ms-grids-time { font-size:11px;color:var(--cos-text-soft);white-space:nowrap;min-width:80px;text-align:right; }' +
        ...
        '.ms-grids-right { width:280px;padding:14px;display:flex;flex-direction:column;gap:8px;' +
            'border-left:1px solid var(--cos-border);overflow:hidden;flex-shrink:0; }' +
        ...
        '.ms-grids-result-name { font-size:12px;color:var(--cos-text);font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis; }' +
        '.ms-grids-result-count { font-size:10px;color:var(--cos-text-soft); }';
    document.head.appendChild(s);
})();
