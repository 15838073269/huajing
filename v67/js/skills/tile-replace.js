...
var TileReplaceSkill = {
    ...
    _tryLoadFromCanvas: function() {
        ...
        img.onload = function() {
            if (self._regionEditing) {
                ...
                self._curr().textureImages[ck] = { image: img, dataURL: dataURL };
            } else {
                // 整张图模式：当作图集导入
            ...
            self._buildTileList();
            if (typeof showToast === 'function') showToast('已从画布导入');
        };
        img.src = dataURL;
    },

    // ===== 画布选中实时联动（窗口常驻时，点选别的画布图片自动切换图集/纹理源） =====
    ...
    _createOverlay: function() {
        ...
        // 恢复保存的尺寸/位置
        var savedW = 800, savedH = 500, savedL = null, savedT = null;
        try {
            var saved = JSON.parse(localStorage.getItem('tr-window-rect-v2'));
            if (saved) {
                var sw = window.innerWidth, sh = window.innerHeight;
                savedW = Math.min(saved.w || 800, sw - 20);
                savedH = Math.min(saved.h || 500, sh - 20);
        ...
        ov.style.left = (savedL !== null ? savedL : 40) + 'px';
        ov.style.top = (savedT !== null ? savedT : 40) + 'px';
        ...
        ov.innerHTML =
            // 标题栏（cos-pwin-hdr）
            '<div class="cos-pwin-hdr tr-header">' +
            '<span class="cos-pwin-hdr-title">地图编辑器</span>' +
            '<div class="cos-pwin-hdr-right">' +
            '<span class="cos-pclose" data-action="close" title="关闭">\u00d7</span>' +
            '</div></div>' +

            // 工具栏（cos-ptoolbar）
            '<div class="cos-ptoolbar tr-toolbar">' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-success tn-grid ac-grid">网格</button>' +
            '<span class="cos-psep"></span>' +
            '<label class="tr-toggle">' +
            '<span class="tr-toggle-label on">画笔</span>' +
            '<input type="checkbox" class="tr-toggle-input" style="display:none;">' +
            '<span class="tr-toggle-switch">' +
            '<span class="tr-toggle-knob"></span></span>' +
            '<span class="tr-toggle-label off">空笔</span></label>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-danger tn-eraser ac-eraser">橡皮擦</button>' +
            '<span class="cos-psep"></span>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-warn tn-select ac-select">框选</button>' +
            '<button class="cos-pbtn cos-pbtn-sm tn-save ac-select" disabled>保存选区</button>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-success tn-godot">Godot导出</button>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-primary tn-canvas-save">保存到画布</button>' +
            '<span class="cos-psep"></span>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-danger tn-clear ac-danger">清空</button>' +
            '<span class="cos-psep"></span>' +
            '<span class="cos-plabel">格:</span>' +
            '<input type="range" class="tr-size-slider" min="16" max="64" step="8" value="16">' +
            '<span class="tr-size-label">16</span>' +
            '<span class="cos-psep"></span>' +
            '<span class="cos-plabel">标记:</span>' +
            '<span class="tr-point-count">0</span>' +
            '</div>' +

            // 主体（画布 + 图层面板 + 贴图面板）
            '<div class="tr-body">' +
            '<div class="tr-canvas-wrap">' +
            '<canvas class="tr-main-canvas"></canvas>' +
            '<div class="tr-status-overlay">' +
            '<span class="tr-status-text"></span></div></div>' +

            '<div class="tr-layer-panel">' +
            '<div class="tr-layer-hdr">图层 <span class="tr-add-layer">+</span></div>' +
            '<div class="tr-layer-list"></div></div>' +

            '<div class="tr-texture-panel">' +
            '<div class="tr-tex-hdr">' +
            '<span class="cos-plabel">贴图</span>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-warn tr-export-btn">导出模板</button>' +
            '<div class="tr-tex-btn-row">' +
            '<button class="cos-pbtn cos-pbtn-sm tr-reset-btn">重置为默认颜色</button>' +
            '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-danger tr-clear-tex-btn">清空贴图</button>' +
            '</div></div>' +
            '<div class="tr-atlas-wrap">' +
            '<div class="tr-hint">图集预览</div>' +
            '<canvas class="tr-atlas-canvas"></canvas>' +
            '<div class="tr-grid-size-row">' +
            '<span class="cos-plabel">列:</span>' +
            '<input class="cos-pinput tr-grid-cols" type="number" min="1" max="20" value="4">' +
            '<span class="cos-plabel">行:</span>' +
            '<input class="cos-pinput tr-grid-rows" type="number" min="1" max="20" value="4">' +
            '</div></div>' +
            '<div class="tr-tile-list"></div>' +
            '</div></div>' +

            // 状态栏（cos-pinfobar）
            '<div class="cos-pinfobar tr-footer">' +
        ...
        if (typeof CosUI !== 'undefined' && CosUI.draggable) {
            CosUI.draggable.bind(ov, this._el.header, {
                storeKey: 'tr-window-rect-v2',
    ...
    _applyStyles: function() {
        ...
        css.textContent =
            // toggle 开关
            '.tr-toggle{display:inline-flex;align-items:center;gap:4px;font-size:11px;cursor:pointer;user-select:none;}' +
            '.tr-toggle-switch{width:28px;height:14px;border-radius:7px;background:var(--cos-accent-soft);position:relative;transition:background 0.2s;flex-shrink:0;}' +
            '.tr-toggle-switch.on{background:rgba(56,189,248,0.45);}' +
            '.tr-toggle-knob{width:12px;height:12px;border-radius:50%;background:var(--cos-accent);position:absolute;top:1px;left:1px;transition:left 0.2s;}' +
            '.tr-toggle-knob.on{left:15px;}' +
            '.tr-size-slider{width:56px;accent-color:var(--cos-accent);}' +
            '.tr-size-label{font-size:11px;min-width:20px;color:var(--cos-text-soft);}' +
            '.tr-toggle-label{font-size:11px;}' +
            '.tr-toggle-label.on{color:var(--cos-accent);}' +
            '.tr-toggle-label.off{color:var(--cos-text-dim);}' +
            '.tr-point-count{color:var(--cos-accent);font-size:11px;min-width:30px;}' +
            '.tr-hint-active{color:var(--cos-yellow);}' +
            // 主体布局
            '.tr-body{display:flex;flex:1;overflow:hidden;min-height:0;}' +
            '.tr-canvas-wrap{flex:1;position:relative;overflow:hidden;}' +
            '.tr-main-canvas{position:absolute;top:0;left:0;}' +
            '.tr-status-overlay{position:absolute;bottom:6px;left:10px;font-size:11px;color:var(--cos-text-dim);pointer-events:none;}' +
            // 图层面板
            '.tr-layer-panel{width:90px;flex-shrink:0;border-left:1px solid var(--cos-border);display:flex;flex-direction:column;overflow:hidden;}' +
            '.tr-layer-hdr{font-size:10px;color:var(--cos-text-dim);padding:4px 6px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}' +
            '.tr-add-layer{color:var(--cos-accent);cursor:pointer;font-size:12px;}' +
            '.tr-layer-list{flex:1;overflow-y:auto;padding:2px 0;}' +
            '.tr-layer-item{display:flex;align-items:center;gap:4px;padding:2px 10px;font-size:11px;cursor:pointer;border-bottom:1px solid rgba(100,160,255,0.03);}' +
            '.tr-layer-item.active{background:var(--cos-accent-soft);}' +
            '.tr-layer-vis{font-size:9px;cursor:pointer;color:var(--cos-text-soft);width:14px;text-align:center;}' +
            '.tr-layer-name{flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}' +
            '.tr-layer-name.active{color:var(--cos-accent);}' +
            '.tr-layer-del{font-size:11px;color:var(--cos-text-dim);cursor:pointer;padding:0 2px;}' +
            // 贴图面板
            '.tr-texture-panel{width:220px;flex-shrink:0;border-left:1px solid var(--cos-border);display:flex;flex-direction:column;overflow:hidden;}' +
            '.tr-tex-hdr{padding:8px 10px;border-bottom:1px solid var(--cos-border);display:flex;align-items:center;justify-content:space-between;flex-shrink:0;flex-wrap:wrap;gap:4px;}' +
            '.tr-tex-btn-row{display:flex;flex-wrap:wrap;gap:4px;padding:4px 10px;flex-shrink:0;}' +
            '.tr-atlas-wrap{padding:8px 10px;border-bottom:1px solid var(--cos-border);flex-shrink:0;}' +
            '.tr-hint{font-size:10px;color:var(--cos-text-dim);margin-bottom:4px;min-height:16px;}' +
            '.tr-atlas-canvas{width:100%;image-rendering:pixelated;border:1px solid var(--cos-border);border-radius:4px;cursor:crosshair;}' +
            '.tr-grid-size-row{display:flex;gap:8px;margin-top:4px;align-items:center;}' +
            '.tr-grid-cols,.tr-grid-rows{width:36px;text-align:center;padding:1px 2px;}' +
            '.tr-tile-list{flex:1;overflow-y:auto;padding:6px 10px;}' +
            // 瓦片列表项
            '.tr-tile-item{display:flex;flex-direction:column;gap:2px;padding:4px 6px;border-bottom:1px solid rgba(100,160,255,0.04);cursor:pointer;transition:background 0.1s;border-radius:4px;}' +
            '.tr-tile-item:hover{background:var(--cos-accent-soft);}' +
            '.tr-tile-item.active{background:rgba(56,189,248,0.12);}' +
            '.tr-tile-preview{width:36px;height:36px;border-radius:4px;border:1px solid var(--cos-border);flex-shrink:0;image-rendering:pixelated;}' +
            '.tr-tile-label{font-size:11px;color:var(--cos-text-soft);flex:1;}' +
            '.tr-tile-top-row{display:flex;align-items:center;gap:8px;width:100%;}' +
            '.tr-tile-rot-row{display:flex;gap:6px;margin-top:2px;padding-left:40px;align-items:center;}' +
            '.tr-tile-rot-btn{font-size:10px;padding:1px 8px;border-radius:4px;cursor:pointer;border:1px solid rgba(56,189,248,0.2);background:rgba(56,189,248,0.08);color:var(--cos-accent);}' +
            // 按钮 active 状态（工具栏按钮高亮）
            '.cos-pbtn.ac-grid.active{background:var(--cos-green);color:#fff;border-color:var(--cos-green);}' +
            '.cos-pbtn.ac-eraser.active{background:var(--cos-red);color:#fff;border-color:var(--cos-red);}' +
            '.cos-pbtn.ac-select.active{background:var(--cos-yellow);color:#1a1a2e;border-color:var(--cos-yellow);}' +
            '.tn-save.disabled{opacity:0.4;cursor:default;}';
    ...
    _cacheElements: function() {
        ...
        this._el = {
            ...
            close: ov.querySelector('[data-action="close"]'),
            ...
            canvasSave: ov.querySelector('.tn-canvas-save'),
    ...
    _bindWindowEvents: function() {
        ...
        // 保存到画布：导出当前框选区域到画布
        el.canvasSave.addEventListener('click', function() {
            self._exportSelection();
        });
    ...
    _loadMultipleTextures: function(files) {
        var self = this;
        this._cfLock = true; // 手动导入贴图后，暂停画布跟随以免覆盖
    ...
    _loadAtlas: function(file) {
        var self = this;
        this._cfLock = true; // 手动导入图集后，暂停画布跟随以免覆盖
    ...
    _renderLayerList: function() {
        ...
        this._layers.forEach(function(l, i) {
            ...
            item.draggable = true;
            item.className = 'tr-layer-item' + (i === self._activeLayerIdx ? ' active' : '');
            // 可见性
            var vis = document.createElement('span');
            vis.className = 'tr-layer-vis';
            ...
            name.className = 'tr-layer-name' + (i === self._activeLayerIdx ? ' active' : '');
    ...
    _buildTileList: function() {
        ...
        keys.forEach(function(k) {
            ...
            topRow.className = 'tr-tile-top-row';
            ...
            if (dirOpts && self._autoRotate) {
                ...
                rotRow.className = 'tr-tile-rot-row';

                var rotBtn = document.createElement('span');
                rotBtn.className = 'tr-tile-rot-btn';
    ...
    _exportSelection: function() {
        ...
        if (typeof CanvasImages !== 'undefined') {
            CanvasImages.place(dataURL, null, null, '选区图 ' + cols + 'x' + rows + ' ' + new Date().toLocaleTimeString(), parentId);
        }
    ...
    _updateHint: function() {
        ...
        if (this._curr().textureMode === 'none') {
            hint.innerHTML = '从画布选中图片后激活即可自动导入贴图';
            return;
        }
        if (this._regionEditing) {
            var label = this._getTileLabel(this._regionEditing);
            if (this._curr().textureMode === 'multi') {
                hint.innerHTML = '<span class="tr-hint-active">▼ 正在交换 ' + label + '</span> 请点击预览图上的其他图片';
            } else {
                hint.innerHTML = '<span class="tr-hint-active">▼ 正在选择 ' + label + '</span> 请点击图集上的格子';
            }
            return;
        }
    ...
    _updateToolbar: function() {
        ...
        if (this._activeTool === 'brush' || this._activeTool === 'empty') {
            ...
            el.toggleSwitch.className = 'tr-toggle-switch' + (isEmpty ? ' on' : '');
            el.toggleKnob.className = 'tr-toggle-knob' + (isEmpty ? ' on' : '');
            if (el.toggleLabels && el.toggleLabels.length >= 2) {
                el.toggleLabels[0].className = 'tr-toggle-label ' + (isEmpty ? 'off' : 'on');
                el.toggleLabels[1].className = 'tr-toggle-label ' + (isEmpty ? 'on' : 'off');
            }
        }
        el.btnEraser.className = 'cos-pbtn cos-pbtn-sm cos-pbtn-danger tn-eraser ac-eraser' + (this._activeTool === 'eraser' ? ' active' : '');
        el.btnSelect.className = 'cos-pbtn cos-pbtn-sm cos-pbtn-warn tn-select ac-select' + (this._activeTool === 'select' ? ' active' : '');
        el.btnGrid.textContent = '网格';
        el.btnGrid.className = 'cos-pbtn cos-pbtn-sm cos-pbtn-success tn-grid ac-grid' + (this._showGrid ? ' active' : '');
        ...
        el.btnSave.className = 'cos-pbtn cos-pbtn-sm tn-save ac-select' + (hasSel ? '' : ' disabled');
    },
    ...
    _exportGodot: function() {
        ...
        if (Object.keys(textures).length === 0) {
            if (typeof showToast === 'function') showToast('没有贴图可导出，请先从画布导入贴图');
            return;
        }
