...
var PromptTemplateSkill = {
    ...
    activate: function(world) {
        ...
        var _afterLoad = function() {
            // 数据加载完成前积攒的待存模板：内存 _templates 已由 addTemplate 预先 unshift，
            // 这里仅清理 pending 队列并触发 IDB 持久化（不再 unshift，避免重复）
            ...
            // 本次新增的模板（生成时面板未打开）在打开时刷新列表
            if (self._pendingRefresh) {
                if (self._renderList) self._renderList();
                if (self._renderSidebar) self._renderSidebar();
                self._pendingRefresh = false;
            }
            SkillSystem.renderSubTools();
        };
        // 关键修复：已加载完成则直接打开面板，避免再次 _loadData 用 IDB 旧数据覆盖内存中新增的提示词
        if (this._dbReadSuccess) {
            _afterLoad();
        } else {
            this._loadData(_afterLoad);
        }
    },
                ...
                get.onsuccess = function() {
                    if (get.result) {
                        ...
                        if (d.templates && d.tags) {
                            ...
                            // 合并而不是覆盖：保留内存中已新增的模板（尚未进 IDB 的），
                            // 仅把 IDB 历史中没有的项追加到末尾，避免 aig-generate 写入的提示词被冲掉
                            var _mem = {};
                            self._templates.forEach(function(t) { _mem[t.text] = true; });
                            d.templates.forEach(function(t) {
                                if (!_mem[t.text]) { self._templates.push(t); _mem[t.text] = true; }
                            });
                        }
                        // v2 格式迁移
                            ...
                            // 同样合并：保留内存新增项
                            var _mem2 = {};
                            self._templates.forEach(function(t) { _mem2[t.text] = true; });
                            d.categories.forEach(function(cat) {
                                if (cat.name) self._tags.push(cat.name);
                                (cat.templates || []).forEach(function(t) {
                                    if (!_mem2[t]) {
                                        self._templates.push({ text: t, tags: cat.name ? [cat.name] : [] });
                                        _mem2[t] = true;
                                    }
                                });
                            });
                    ...
                    // 主动 flush 积攒的待存模板：无论加载由 activate 还是 addTemplate 触发，
                    // 数据就绪后都立即把 pending 刷入 _templates 并持久化，避免提示词卡在队列不进面板。
                    if (self._pendingTemplates && self._pendingTemplates.length) {
                        var _pts = self._pendingTemplates;
                        self._pendingTemplates = [];
                        try { localStorage.removeItem('pt_pending_backup'); } catch(e) {}
                        // 内存 _templates 已含这些项（addTemplate 预先 unshift），此处仅用于触发 IDB 持久化
                        self._saveData();
                        if (self._managerEl && self._managerEl.style.display !== 'none') {
                            if (self._renderList) self._renderList();
                            if (self._renderSidebar) self._renderSidebar();
                        }
                    }
                    // 同步 localStorage 主存储，保证与内存一致
                    try { localStorage.setItem('pt_templates_mem', JSON.stringify(self._templates)); } catch(e) {}
                    if (cb) cb();
                };
    ...
    _createManager: function() {
        ...
        panel.className = 'cos-pwin pt-manager';
        panel.innerHTML =
            '<div class="cos-pwin-hdr pt-header">' +
                '<span class="cos-pwin-hdr-title">提示词模板</span>' +
                '<div class="cos-pwin-hdr-right">' +
                    '<button class="cos-pbtn cos-pbtn-sm" id="ptImport" title="导入">导入</button>' +
                    '<button class="cos-pbtn cos-pbtn-sm" id="ptExport" title="导出">导出</button>' +
                    '<span class="cos-pclose" data-action="close" title="关闭">&times;</span>' +
                '</div>' +
            '</div>' +
            '<div class="pt-mgr-body">' +
                ...
                '<div class="pt-main">' +
                    '<div class="pt-add">' +
                        '<textarea class="cos-ptextarea pt-add-input" id="ptAddText" placeholder="输入模板，一行一条&#10;用 [中括号] 标记可变部分" rows="3"></textarea>' +
                        '<button class="cos-pbtn cos-pbtn-primary pt-add-btn" id="ptAddBtn">添加</button>' +
                    '</div>' +
                    ...
                    '<div class="pt-zoom-bar">' +
                        ...
                        '<span class="pt-zoom-label large">A</span>' +
                    '</div>' +
                '</div>' +
            '</div>' +
            '<div class="cos-pinfobar pt-footer">' +
                '<span></span>' +
                '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-danger" id="ptClear" title="清空全部模板，不可恢复">清空全部</button>' +
            '</div>' +
        ...
        panel.addEventListener('mousedown', function() {
            ...
            panel.style.zIndex = tz;
        });

        // 拖拽（使用公共 CosUI.draggable）
        var header = panel.querySelector('.pt-header');
        if (typeof CosUI !== 'undefined' && CosUI.draggable) {
            CosUI.draggable.bind(panel, header, {
                storeKey: 'pt-window-rect',
    ...
    _renderSidebar: function() {
        ...
        html += '</div>';
        html += '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-secondary pt-btn-add-cat" id="ptAddTag">+ 新标签</button>';
    ...
    _renderList: function() {
        ...
        filtered.forEach(function(tpl, fi) {
            ...
            html += '<div class="pt-item-actions">' +
                '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-primary pt-copy-btn" data-idx="' + realIdx + '">复制</button>' +
                '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-success pt-send-btn" data-idx="' + realIdx + '" title="发送到AI生图">→生</button>' +
                '<button class="cos-pbtn cos-pbtn-sm cos-pbtn-danger pt-del-btn" data-idx="' + realIdx + '">关</button>' +
            '</div>';
            html += '</div>';
        });
        list.innerHTML = html;
    ...
    addTemplate: function(text, tags) {
        ...
        var item = { text: text.trim(), tags: tags || [] };
        // 关键修复：立即写入内存，保证模板面板立即可见，不依赖 IndexedDB 是否已加载完成。
        // 之前必须等 _dbReadSuccess 才写入，而 aig-generate 触发 _ensureLoaded + activate 的
        // 两次 _loadData 会用旧 IDB 数据覆盖 _templates，导致刚写入的提示词被冲掉、永远进不了面板。
        this._templates.unshift(item);
        // 同步持久化兜底（localStorage 主存储之一），即使不激活、不触发 _loadData 也不会丢
        try { localStorage.setItem('pt_templates_mem', JSON.stringify(this._templates)); } catch(e) {}
        // 若面板当前可见，立即刷新列表
        ...
        // IndexedDB 未就绪：进入 pending 队列（仅用于后台同步），并触发加载持久化
        if (!this._dbReadSuccess) {
            if (!this._pendingTemplates) this._pendingTemplates = [];
            this._pendingTemplates.push(item);
            try { localStorage.setItem('pt_pending_backup', JSON.stringify(this._pendingTemplates)); } catch(e) {}
            this._ensureLoaded();
            return;
        }
        // 已就绪：直接持久化到 IndexedDB
        this._saveData();
    },
...
(function() {
    ...
    s.textContent =
        ...
        '.pt-sidebar { width:120px;flex-shrink:0;display:flex;flex-direction:column;' +
            'border-right:1px solid var(--cos-border);background:var(--cos-surface); }' +
        ...
        '.pt-tag-item { display:flex;align-items:center;justify-content:space-between;' +
            'padding:7px 12px;cursor:pointer;transition:all .15s;font-size:12px;color:var(--cos-text-soft);gap:4px; }' +
        '.pt-tag-item:hover { background:var(--cos-accent-soft);color:var(--cos-text); }' +
        '.pt-tag-item.active { background:var(--cos-accent-soft);color:var(--cos-accent);border-right:2px solid var(--cos-accent); }' +
        '.pt-tag-name { flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }' +
        '.pt-tag-count { font-size:10px;color:var(--cos-text-dim);background:var(--cos-accent-soft);' +
            'padding:1px 6px;border-radius:10px;flex-shrink:0; }' +
        '.pt-tag-del { display:none;background:none;border:none;color:var(--cos-text-dim);font-size:12px;' +
            'cursor:pointer;padding:0 3px;border-radius:5px;transition:all .15s;flex-shrink:0;line-height:1; }' +
        '.pt-tag-del:hover { color:var(--cos-red);background:rgba(239,68,68,.1); }' +
        ...
        '.pt-cat-new-input { width:calc(100% - 8px);margin:4px;box-sizing:border-box;' +
            'background:var(--cos-surface);border:1px solid var(--cos-border-warm);border-radius:6px;' +
            'color:var(--cos-text);padding:6px 8px;font-size:11px;outline:none;font-family:inherit; }' +
        '.pt-cat-new-input:focus { border-color:var(--cos-accent-glow); }' +
        '.pt-cat-new-input::placeholder { color:var(--cos-text-dim); }' +

        /* 分隔条 */
        '.pt-resizer { width:4px;cursor:col-resize;background:transparent;flex-shrink:0;transition:background .15s; }' +
        '.pt-resizer:hover, .pt-resizer:active { background:var(--cos-accent-glow); }' +

        /* 右侧 */
        '.pt-main { flex:1;display:flex;flex-direction:column;min-width:0;overflow:hidden; }' +
        '.pt-add { padding:8px 10px;border-bottom:1px solid var(--cos-border);flex-shrink:0; }' +
        '.pt-add-input { width:100%;box-sizing:border-box;font-size:11px;resize:none;line-height:1.5; }' +
        '.pt-add-btn { width:100%;margin-top:4px; }' +

        /* 列表 */
        '.pt-list { flex:1;overflow-y:auto;min-height:0; }' +
        '.pt-empty { text-align:center;color:var(--cos-text-dim);padding:30px 10px;font-size:12px; }' +
        ...
        '.pt-item:hover { background:var(--cos-accent-soft); }' +
        '.pt-item-body { display:flex;flex-wrap:wrap;align-items:center;gap:3px;line-height:1.6;min-width:0; }' +
        '.pt-text { color:var(--cos-text);word-break:break-all; }' +
        '.pt-slot { background:var(--cos-accent-soft);border:1px solid var(--cos-border-warm);' +
            'border-radius:5px;padding:1px 6px;color:var(--cos-accent);width:80px;' +
            'outline:none;transition:border-color .15s;font-family:inherit;flex-shrink:0; }' +
        '.pt-slot:focus { border-color:var(--cos-accent-glow);background:rgba(56,189,248,.12); }' +
        ...
        '.pt-item-tag { display:inline-flex;align-items:center;gap:2px;padding:2px 8px;' +
            'background:var(--cos-accent-soft);border:1px solid var(--cos-border-warm);' +
            'border-radius:5px;color:var(--cos-accent);font-size:11px;cursor:default; }' +
        '.pt-item-tag-del { background:none;border:none;color:var(--cos-text-dim);font-size:12px;cursor:pointer;' +
            'padding:0 1px;line-height:1;transition:color .15s; }' +
        '.pt-item-tag-del:hover { color:var(--cos-red); }' +
        '.pt-item-tag-add { background:none;border:1px dashed var(--cos-border);border-radius:5px;' +
            'color:var(--cos-text-dim);font-size:11px;cursor:pointer;padding:2px 8px;transition:all .15s; }' +
        '.pt-item-tag-add:hover { color:var(--cos-text-soft);border-color:var(--cos-border-warm); }' +

        /* 标签下拉菜单 */
        '.pt-tag-menu { position:absolute;left:0;bottom:100%;margin-bottom:2px;z-index:10;' +
            'background:var(--cos-world);border:1px solid var(--cos-border);border-radius:8px;' +
            'box-shadow:var(--cos-shadow);min-width:80px;overflow:hidden; }' +
        '.pt-tag-menu-item { padding:6px 12px;font-size:11px;color:var(--cos-text);cursor:pointer;transition:background .15s; }' +
        '.pt-tag-menu-item:hover { background:var(--cos-accent-soft);color:var(--cos-accent); }' +

        /* 缩放 */
        '.pt-zoom-bar { display:flex;align-items:center;gap:6px;padding:4px 12px;' +
            'border-top:1px solid var(--cos-border);flex-shrink:0; }' +
        '.pt-zoom-label { color:var(--cos-text-dim);font-size:10px;flex-shrink:0; }' +
        '.pt-zoom-label.large { font-size:16px; }' +
        ...
        '.pt-zoom-slider::-webkit-slider-thumb { -webkit-appearance:none;appearance:none;' +
            'width:12px;height:12px;border-radius:50%;background:var(--cos-accent-glow);' +
            'border:1px solid var(--cos-border-warm);cursor:pointer; }' +

        /* 滚动条 */
        '.pt-list::-webkit-scrollbar, .pt-tag-list::-webkit-scrollbar { width:3px; }' +
        '.pt-list::-webkit-scrollbar-thumb, .pt-tag-list::-webkit-scrollbar-thumb { background:var(--cos-accent-soft);border-radius:2px; }' +

        /* 侧栏新增标签按钮 */
        '.pt-btn-add-cat { background:var(--cos-accent-soft);border:1px dashed var(--cos-border);color:var(--cos-text-dim); }' +
        '.pt-btn-add-cat:hover { background:rgba(56,189,248,.06);color:var(--cos-text-soft); }';
