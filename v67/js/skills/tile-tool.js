...
var TileToolSkill = {
    ...
    _getCSS: function() {
        return [
            /* 模式标签栏（插件专属，嵌于标题栏内） */
            '.tt-mode-tabs { display:flex; gap:4px; background:var(--cos-surface); border-radius:10px; padding:3px; }',
            '.tt-mode-tab { padding:7px 18px; border:none; border-radius:8px; cursor:pointer; background:transparent; color:var(--cos-text-soft); font-size:13px; font-weight:500; transition:all .15s; }',
            '.tt-mode-tab.active { background:var(--cos-accent); color:#fff; }',
            '.tt-mode-tab:hover:not(.active) { background:rgba(255,255,255,.08); }',
            '.tt-mode-tab:active { transform:scale(0.92); }',
            /* App 布局 */
            ...
            /* 步骤标题 */
            '.tt-step-title { font-size:13px; font-weight:bold; color:var(--cos-accent); margin:0 0 8px; padding:6px 10px; background:var(--cos-accent-soft); border-radius:8px; border-left:3px solid var(--cos-accent); }',
            '.tt-step-num { display:inline-block; width:22px; height:22px; line-height:22px; text-align:center; background:var(--cos-accent); color:#fff; border-radius:50%; font-size:12px; margin-right:6px; }',
            /* 按钮组 */
            '.tt-btn-group { display:flex; gap:6px; }',
            '.tt-btn-group .cos-pbtn { flex:1; }',
            /* 输入组 */
            '.tt-input-group { margin-bottom:9px; }',
            '.tt-input-group label { display:block; font-size:11px; color:var(--cos-text-soft); margin-bottom:3px; }',
            '.tt-input-group input[type="range"] { width:100%; accent-color:var(--cos-accent); }',
            '.tt-input-group input[type="number"], .tt-input-group input[type="text"], .tt-input-group select { width:100%; padding:5px 9px; background:var(--cos-surface); border:1px solid var(--cos-border); border-radius:6px; color:var(--cos-text); font-size:12px; }',
            /* 开关 */
            ...
            '.tt-toggle input:checked + .tt-toggle-slider { background:var(--cos-accent); }',
            ...
            '.tt-grid-row label:first-child { font-size:11px; color:var(--cos-text-soft); flex-shrink:0; }',
            '.tt-grid-row input[type="number"] { width:70px; padding:4px 8px; background:var(--cos-surface); border:1px solid var(--cos-border); border-radius:6px; color:var(--cos-text); font-size:12px; text-align:right; }',
            '.tt-grid-input-wrap { display:flex; align-items:center; gap:4px; }',
            '.tt-grid-input-wrap span { color:var(--cos-text-dim); font-size:11px; }',
            /* 滑动行 */
            '.tt-range-row { display:flex; align-items:center; gap:8px; }',
            '.tt-range-row input[type="range"] { flex:1; }',
            '.tt-range-val { min-width:36px; text-align:center; font-size:11px; color:var(--cos-accent); font-weight:600; }',
            /* 上传区 */
            '.tt-upload-zone { border:2px dashed var(--cos-border); border-radius:12px; padding:24px 16px; text-align:center; cursor:pointer; transition:all .15s; margin-bottom:10px; }',
            '.tt-upload-zone:hover, .tt-upload-zone.dragover { border-color:var(--cos-accent); background:var(--cos-accent-soft); }',
            '.tt-upload-zone .tt-icon { font-size:32px; margin-bottom:6px; }',
            '.tt-upload-zone p { font-size:12px; color:var(--cos-text-soft); }',
            ...
            '.tt-canvas-hint { position:absolute; top:0; left:0; right:0; padding:4px 10px; background:rgba(0,0,0,0.7); color:var(--cos-text-soft); font-size:10px; text-align:center; pointer-events:none; z-index:15; backdrop-filter:blur(4px); }',
            '.tt-overlay-canvas { position:absolute; top:0; left:0; cursor:crosshair; }',
            /* 信息栏 */
            '.tt-info-bar { position:absolute; bottom:12px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,.7); backdrop-filter:blur(8px); padding:5px 14px; border-radius:22px; font-size:11px; color:var(--cos-text-soft); display:flex; gap:14px; z-index:10; }',
            '.tt-info-bar span { display:flex; align-items:center; gap:4px; }',
            '.tt-info-bar .tt-val { color:var(--cos-accent); font-weight:600; }',
            /* 复选框行 */
            '.tt-checkbox-row { display:flex; align-items:center; gap:8px; font-size:12px; cursor:pointer; margin-bottom:7px; }',
            '.tt-checkbox-row input { accent-color:var(--cos-accent); }',
            /* 取色行 */
            '.tt-color-pick-row { display:flex; align-items:center; gap:8px; }',
            '.tt-color-pick-row input[type="color"] { width:30px; height:26px; border:none; border-radius:6px; cursor:pointer; background:transparent; }',
            /* 区域列表 */
            '.tt-region-list { max-height:180px; overflow-y:auto; }',
            '.tt-region-item { display:flex; align-items:center; gap:6px; padding:4px 5px; border-radius:6px; font-size:11px; cursor:pointer; transition:.15s; border-left:3px solid transparent; }',
            '.tt-region-item:hover { background:rgba(255,255,255,.06); }',
            '.tt-region-item.selected { background:var(--cos-accent-soft); border-left-color:var(--cos-accent); }',
            ...
            '.tt-region-item .tt-info { flex:1; color:var(--cos-text-soft); }',
            '.tt-region-item .tt-del { width:18px; height:18px; background:transparent; border:none; color:var(--cos-text-soft); cursor:pointer; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:14px; }',
            '.tt-region-item .tt-del:hover { background:var(--cos-accent); color:#fff; }',
            '.tt-region-item .tt-inner-cb { width:16px; height:16px; accent-color:#00c853; cursor:pointer; flex-shrink:0; }',
            /* Toast */
            '.tt-toast { position:fixed; top:20px; right:20px; background:var(--cos-surface); color:#fff; padding:9px 18px; border-radius:10px; font-size:12px; z-index:99999; transform:translateX(120%); transition:.3s; border-left:3px solid #00c853; pointer-events:none; }',
            '.tt-toast.show { transform:translateX(0); }',
            '.tt-toast.error { border-left-color:var(--cos-accent); }',
            /* 空状态 */
            '.tt-empty-state { text-align:center; color:var(--cos-text-soft); }',
            ...
            '.tt-merge-item { position:relative; border-radius:8px; overflow:hidden; background:var(--cos-surface); }',
            ...
            '.tt-merge-item .tt-del-btn { position:absolute; top:2px; right:2px; width:18px; height:18px; background:var(--cos-accent); color:#fff; border:none; border-radius:50%; font-size:11px; cursor:pointer; display:none; align-items:center; justify-content:center; }',
            '.tt-merge-item:hover .tt-del-btn { display:flex; }',
            /* 放大镜 */
            '.tt-magnifier { display:none; position:fixed; width:140px; height:140px; border-radius:50%; border:3px solid var(--cos-accent); box-shadow:0 4px 20px rgba(0,0,0,.6); pointer-events:none; z-index:99998; overflow:hidden; background:#000; }',
            ...
            '.tt-sub-tab{flex:1;padding:5px 8px;border-radius:6px;border:1px solid var(--cos-border);background:var(--cos-surface);color:var(--cos-text-soft);font-size:12px;cursor:pointer;text-align:center;transition:all .15s;}',
            '.tt-sub-tab:hover{border-color:var(--cos-accent);color:var(--cos-accent);}',
            '.tt-sub-tab.active{background:var(--cos-accent);border-color:var(--cos-accent);color:#fff;}'
        ].join('\n');
    },
    ...
    _createOverlay: function() {
        ...
        // Create overlay container - 使用公共 cos-pwin
        ...
        // 恢复保存的尺寸/位置（使用公共库逻辑）
        var savedW = 900, savedH = 600, savedL = null, savedT = null;
        try {
            var saved = JSON.parse(localStorage.getItem('tt-window-size'));
            if (saved) {
                var sw = window.innerWidth, sh = window.innerHeight;
                savedW = Math.min(saved.w || 900, sw - 20);
                savedH = Math.min(saved.h || 600, sh - 20);
        ...
        header.innerHTML =
            '<span class="cos-pwin-hdr-title">扣图</span>' +
        ...
        if (typeof WindowHelper !== 'undefined') {
            WindowHelper.makeResizable(overlay, { minWidth: 600, minHeight: 400, storeKey: 'tt-window-size' });
        }

        // 拖拽（使用公共 CosUI.draggable）
        if (typeof CosUI !== 'undefined' && CosUI.draggable) {
            CosUI.draggable.bind(overlay, header, {
                storeKey: 'tt-window-size',
    ...
    _buildSidebarHTML: function() {
        ...
        '<div class="tt-panel active" id="ttSplitPanel">' +
            ...
            '<div id="ttGridPanel" style="display:none">' +
                '<div class="tt-step-title"><span class="tt-step-num">1</span> 方形分割设置</div>' +
                ...
                '<div style="display:flex;justify-content:flex-end;margin-top:8px"><button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" data-action="gridSplit">方形分割</button></div>' +
            '</div>' +
            '<!-- IRREGULAR MODE -->' +
            '<div id="ttIrregularPanel">' +
                '<div class="cos-psection">' +
                    '<div class="tt-step-title"><span class="tt-step-num">1</span> 标记背景色/轮廓色 & 异形检测</div>' +
                    '<p style="font-size:11px;color:var(--cos-text-dim);margin-bottom:7px">先标注背景色和轮廓色，再检测</p>' +
                    '<div class="tt-input-group">' +
                        '<label>背景色</label>' +
                        '<div class="tt-color-pick-row">' +
                            '<input type="color" id="irBgColor" value="#ffffff">' +
                            '<span id="irBgColorHex" style="font-size:11px;color:var(--cos-text-dim)">#FFFFFF</span>' +
                            '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="irColorPick" data-pick-type="bg" style="margin-left:auto">取色</button>' +
                        '</div>' +
                    '</div>' +
                    '<div class="tt-input-group">' +
                        '<label>轮廓色</label>' +
                        '<div class="tt-color-pick-row">' +
                            ...
                            '<span id="irOutlineColorHex" style="font-size:11px;color:var(--cos-text-dim)">#000000</span>' +
                            '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="irColorPick" data-pick-type="outline" style="margin-left:auto">取色</button>' +
                        '</div>' +
                    '</div>' +
                    '<div class="tt-input-group">' +
                        '<label>轮廓色容差</label>' +
                    ...
                    '<div class="tt-input-group">' +
                        ...
                        '</div>' +
                    '</div>' +
                    '<button class="cos-pbtn cos-pbtn-primary" data-action="smartDetect" style="width:100%;margin-top:8px">粗略扣图</button>' +
                '</div>' +
                '<div id="ttIrregularSteps">' +
                '<div class="cos-psection">' +
                    '<div class="tt-step-title"><span class="tt-step-num">2</span> 选择内扣图素材</div>' +
                    '<div class="tt-btn-group" style="margin-bottom:5px">' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="selectAllRegions" data-select="true">全选</button>' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="selectAllRegions" data-select="false">全不选</button>' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="invertRegionSelection">反选</button>' +
                    '</div>' +
                    '<div class="tt-region-list" id="ttRegionList">' +
                        '<p style="font-size:11px;color:var(--cos-text-dim);text-align:center;padding:10px 0">点击上方按钮开始检测</p>' +
                    '</div>' +
                    '<div class="tt-btn-group" style="margin-top:7px">' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="clearAllRegions">清空</button>' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="undoLastRegion">撤销</button>' +
                    '</div>' +
                '</div>' +
                '<div class="cos-psection" id="ttInnerBgSection">' +
                    '<div class="tt-step-title"><span class="tt-step-num">3</span> 内扣设置 & 去除内部背景</div>' +
                    '<div class="tt-input-group">' +
                        '<label>内部背景色</label>' +
                        '<div class="tt-color-pick-row">' +
                            '<input type="color" id="innerBgColor" value="#ffffff">' +
                            '<span id="innerBgColorHex" style="font-size:11px;color:var(--cos-text-dim)">#FFFFFF</span>' +
                            '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="irColorPick" data-pick-type="innerBg" style="margin-left:auto">取色</button>' +
                        '</div>' +
                    '</div>' +
                    '<div class="tt-input-group">' +
                        ...
                        '<div class="tt-color-pick-row">' +
                            ...
                            '<span id="innerOutlineColorHex" style="font-size:11px;color:var(--cos-text-dim)">#000000</span>' +
                            '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="irColorPick" data-pick-type="innerOutline" style="margin-left:auto">取色</button>' +
                        '</div>' +
                    '</div>' +
                    '<div class="tt-input-group">' +
                        '<label>内部容差</label>' +
                    ...
                    '<div class="tt-input-group">' +
                        ...
                        '</div>' +
                    '</div>' +
                    '<button class="cos-pbtn cos-pbtn-primary" data-action="applyInnerBgRemove" style="width:100%;margin-top:8px">精致扣图</button>' +
                '</div>' +
                '<div class="cos-psection" id="ttRestoreSection">' +
                    '<div class="tt-step-title"><span class="tt-step-num">4</span> 套索框选 / 恢复误扣</div>' +
                    '<div style="font-size:11px;color:var(--cos-text-dim);margin-bottom:5px">用套索圈出不要内扣的区域，点击确认恢复像素</div>' +
                    '<div class="tt-btn-group" style="margin-bottom:5px;flex-wrap:wrap;gap:4px">' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="lassoMode" data-lasso="lasso">套索</button>' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="lassoMode" data-lasso="eraser">擦除</button>' +
                        '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="clearLasso">清除</button>' +
                    '</div>' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" data-action="applyRestore" style="width:100%">确认恢复</button>' +
                '</div>' +
                '</div>' +
            '</div>' +
            '<!-- COMMON OUTPUT -->' +
            '<div class="cos-psection">' +
                '<div class="tt-step-title"><span class="tt-step-num">5</span> 输出 & 下载</div>' +
                '<div class="tt-input-group">' +
                    '<label>输出格式</label>' +
                    '<select id="splitFormat" class="cos-pinput">' +
                ...
                '<div class="tt-btn-group" style="flex-wrap:wrap;gap:4px;margin-top:8px">' +
                    '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" data-action="splitAndDownload" id="ttSplitBtn" disabled>拆分下载</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm tt-canvas-split-export">保存到画布</button>' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" data-action="pushToMerge" id="ttPushMergeBtn" disabled>推送合并</button>' +
                '</div>' +
                '<div class="tt-btn-group" style="margin-top:6px">' +
                    '<button class="cos-pbtn cos-pbtn-primary cos-pbtn-sm" data-action="exportGodotScene" id="ttGodotBtn" disabled>🎮 导出 Godot 场景</button>' +
                '</div>' +
            '</div>' +
        '</div>' +
        '<!-- MERGE PANEL -->' +
        '<div class="tt-panel" id="ttMergePanel">' +
            '<div class="cos-psection" style="font-size:11px;color:var(--cos-text-soft);padding:8px 10px;">在画布中选中图片后激活本插件，图片自动添加到合并列表</div>' +
            '<div class="cos-psection">' +
                '<div class="cos-psection-title">排列设置</div>' +
                '<div class="tt-input-group">' +
                    '<label>每行列数</label>' +
                    '<div class="tt-range-row">' +
            ...
            '</div>' +
            '<div class="cos-psection">' +
                '<div class="cos-psection-title">输出设置</div>' +
                '<div class="tt-input-group">' +
                    '<label>输出格式</label>' +
                    '<select id="mergeFormat" class="cos-pinput">' +
                        '<option value="png">PNG (无损)</option>' +
                        '<option value="webp">WebP (推荐)</option>' +
                    '</select>' +
                '</div>' +
                '<div class="tt-btn-group" style="margin-top:7px">' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="sortMerge" data-sort-by="name">🔤 按名称</button>' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="sortMerge" data-sort-by="size">📐 按尺寸</button>' +
                '</div>' +
                '<div class="tt-btn-group" style="margin-top:5px">' +
                    '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm" data-action="clearMergeItems">🗑️ 清空</button>' +
                '</div>' +
            '</div>' +
            '<div class="cos-psection" style="display:flex;gap:6px;flex-wrap:wrap;">' +
                '<button class="cos-pbtn cos-pbtn-success cos-pbtn-sm" data-action="mergeAndDownload" id="ttMergeBtn" disabled>🔗 合并并下载</button>' +
                '<button class="cos-pbtn cos-pbtn-secondary cos-pbtn-sm tt-canvas-merge-export">保存到画布</button>' +
            '</div>' +
        '</div>';
    },
    ...
    _bindEvents: function(overlay) {
        ...
        // 拆分保存到画布
        var canvasSplitExp = overlay.querySelector('.tt-canvas-split-export');
        if (canvasSplitExp) {
            canvasSplitExp.addEventListener('click', function() {
                if (!self.state.originalImage) { self._showToast('请先导入图片'); return; }
                ...
                var parentId = CanvasImages ? CanvasImages.getSelectedId() : null;
                if (typeof CanvasImages !== 'undefined') {
                    CanvasImages.place(dataURL, null, null, '拆分图 ' + new Date().toLocaleTimeString(), parentId);
                }
                self._showToast('已保存到画布');
            });
        }

        // 合并保存到画布
        var canvasMergeExp = overlay.querySelector('.tt-canvas-merge-export');
        if (canvasMergeExp) {
            canvasMergeExp.addEventListener('click', function() {
                if (self.state.mergeImages.length === 0) { self._showToast('请先导入素材'); return; }
    ...
    _tryLoadFromCanvas: function() {
        ...
        if (dataURL) {
            this._loadSplitImageDataURL(dataURL);
            this._showToast('已从画布导入图片');
        }
    },

    // ===== 画布选中实时联动（窗口常驻时，点选别的画布图片自动切换拆分/合并源） =====
    ...
    _loadSplitImage: function(file) {
        var self = this;
        this._cfLock = true; // 手动导入图片后暂停画布跟随以免覆盖
    ...
    _updateRegionListUI: function() {
        ...
        if (this.state.regions.length === 0) {
            container.innerHTML = '<p style="font-size:11px;color:var(--cos-text-dim);text-align:center;padding:10px 0">点击上方按钮开始检测</p>';
            return;
        }
        container.innerHTML = '';
    ...
    _splitAndDownload: function() {
        ...
        if (typeof CanvasImages !== 'undefined') {
            CanvasImages.place(srcCanvas.toDataURL('image/png'), null, null, '拆分原图 ' + new Date().toLocaleTimeString(), parentId);
        }
    ...
    _handleMergeDataURL: function(dataURL) {
        ...
        var img = new Image();
        img.onload = function() {
            self.state.mergeImages.push({ name: '画布-' + (self.state.mergeImages.length + 1), img: img, dataUrl: dataURL });
            self._updateMergePreview();
            self._showToast('已添加画布图片');
        };
        img.src = dataURL;
    },
    ...
    _mergeAndDownload: function(cloudOnly) {
        ...
        if (cloudOnly && typeof CanvasImages !== 'undefined') {
            var parentId = CanvasImages.getSelectedId();
            CanvasImages.place(dataUrl, null, null, '合拼图 ' + new Date().toLocaleTimeString(), parentId);
        }
        this._showToast(cloudOnly ? '已保存到画布' : '合并完成! (' + canvasW + '\u00d7' + canvasH + ')');
    },
