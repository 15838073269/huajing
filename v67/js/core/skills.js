    ...
    function showStore() {
        ...
        zoneMeta.forEach(function(meta) {
            ...
            items.forEach(function(item) {
                ...
                var itemColor = meta.cssColor;
                if (item.disabled) itemColor = '#666';
                var isActive = (item.id === activeSkill);
                html += _bagItemHtml(item, itemColor, num, isActive);
            });
            html += '</div></div>';
        });
        ...
        function _bagItemHtml(item, zoneColor, num, isActive) {
            ...
            var activeStyle = isActive ?
                ('box-shadow:0 0 10px ' + c + '80,inset 0 0 6px ' + c + '30;' +
                 'border:2px solid ' + c + ';') : '';
            return '<div class="cos-bag-item' + (isActive ? ' cos-bag-item-active' : '') + '" draggable="true" data-id="' + item.id + '" ' +
                ...
                'opacity:' + (dimmed ? '0.4' : '1') + ';' + activeStyle + '" title="' + item.skill.name + '">' +
                '<span style="font-size:16px;font-weight:bold;">' + icon + '</span>' +
                '<span style="font-size:8px;color:' + (isActive ? c : '#94a3b8') + ';margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:48px;text-align:center;">' + item.skill.name + '</span>' +
                (num ? '<span style="position:absolute;right:1px;bottom:1px;font-size:7px;color:#475569;">' + num + '</span>' : '') +
                (isActive ? '<span style="position:absolute;top:1px;left:1px;width:6px;height:6px;border-radius:50%;background:' + c + ';box-shadow:0 0 4px ' + c + ';"></span>' : '') +
                '</div>';
        }
