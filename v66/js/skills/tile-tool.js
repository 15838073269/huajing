...
var TileToolSkill = {
    ...
    _getCSS: function() {
        return [
            ':root {',
            '  --bg: rgba(15,25,50,0.95); --bg2: rgba(20,35,70,0.8); --bg3: rgba(10,18,40,0.9);',
            '  --accent: #38bdf8; --accent2: #0ea5e9; --text: #e8edf5; --text2: #94a3b8;',
            '  --border: rgba(100,160,255,0.15); --success: #00c853; --warn: #ffab00;',
            '}',
            '.tt-overlay { position:fixed; width:900px; height:600px; z-index:9999; display:flex; flex-direction:column; background:var(--bg); color:var(--text); font-family:"Segoe UI",system-ui,sans-serif; border-radius:12px; box-shadow:0 8px 40px rgba(0,0,0,.6); overflow:auto; user-select:none; resize:both; min-width:600px; min-height:400px; }',
            '.tt-header { display:flex; align-items:center; justify-content:space-between; padding:8px 16px; background:var(--bg2); border-bottom:1px solid var(--border); flex-shrink:0; cursor:move; user-select:none; }',
            '.tt-header h1 { font-size:18px; background:linear-gradient(135deg,#38bdf8,#7dd3fc); -webkit-background-clip:text; -webkit-text-fill-color:transparent; margin:0; }',
            '.tt-mode-tabs { display:flex; gap:4px; background:var(--bg); border-radius:10px; padding:3px; }',
            '.tt-mode-tab { padding:7px 18px; border:none; border-radius:8px; cursor:pointer; background:transparent; color:#94a3b8; font-size:13px; font-weight:500; transition:all .15s; }',
            '.tt-mode-tab.active { background:#38bdf8; color:#fff; }',
            '.tt-mode-tab:hover:not(.active) { background:rgba(255,255,255,.08); }',
            '.tt-mode-tab:active { transform:scale(0.92); }',
            '.tt-close-btn { background:rgba(220,80,60,.2); border:1px solid rgba(220,80,60,.3); color:#e87060; border-radius:8px; padding:5px 14px; cursor:pointer; font-size:13px; margin-left:12px; transition:all .15s; }',
            '.tt-close-btn:hover { background:rgba(220,80,60,.4); }',
            '.tt-close-btn:active { transform:scale(0.92); }',
            '.tt-app { display:flex; flex:1; overflow:hidden; min-height:0; }',
            '.tt-sidebar { width:300px; min-width:300px; background:var(--bg2); border-right:1px solid var(--border); overflow-y:auto; padding:14px; }',
            ...
            '.tt-section { margin-bottom:16px; padding:10px; border:1px dashed rgba(100,160,255,0.2); border-radius:10px; background:rgba(255,255,255,.02); }',
            '.tt-section-title { font-size:11px; text-transform:uppercase; letter-spacing:1px; color:#94a3b8; margin-bottom:7px; display:flex; align-items:center; gap:6px; }',
            '.tt-section-title::before { content:""; width:3px; height:12px; background:#38bdf8; border-radius:2px; }',
            '.tt-step-title { font-size:13px; font-weight:bold; color:#38bdf8; margin:0 0 8px; padding:6px 10px; background:rgba(56,189,248,0.1); border-radius:8px; border-left:3px solid #38bdf8; }',
            '.tt-step-num { display:inline-block; width:22px; height:22px; line-height:22px; text-align:center; background:#38bdf8; color:#fff; border-radius:50%; font-size:12px; margin-right:6px; }',
            '.tt-btn { display:inline-flex; align-items:center; justify-content:center; gap:6px; padding:7px 14px; border:none; border-radius:8px; cursor:pointer; font-size:12px; font-weight:500; transition:all .15s; width:100%; }',
            '.tt-btn-primary { background:#38bdf8; color:#fff; }',
            '.tt-btn-primary:hover { background:#0ea5e9; transform:translateY(-1px); }',
            '.tt-btn:active { transform:scale(0.92); }',
            '.tt-btn-secondary { background:rgba(255,255,255,.08); color:#e8edf5; border:1px solid rgba(100,160,255,0.15); }',
            ...
            '.tt-input-group label { display:block; font-size:11px; color:#94a3b8; margin-bottom:3px; }',
            '.tt-input-group input[type="range"] { width:100%; accent-color:#38bdf8; }',
            '.tt-input-group input[type="number"], .tt-input-group input[type="text"], .tt-input-group select { width:100%; padding:5px 9px; background:var(--bg); border:1px solid var(--border); border-radius:6px; color:#e8edf5; font-size:12px; }',
            ...
            '.tt-toggle input:checked + .tt-toggle-slider { background:#38bdf8; }',
            ...
            '.tt-grid-row label:first-child { font-size:11px; color:#94a3b8; flex-shrink:0; }',
            '.tt-grid-row input[type="number"] { width:70px; padding:4px 8px; background:var(--bg); border:1px solid var(--border); border-radius:6px; color:#e8edf5; font-size:12px; text-align:right; }',
            '.tt-grid-input-wrap { display:flex; align-items:center; gap:4px; }',
            '.tt-grid-input-wrap span { color:#64748b; font-size:11px; }',
            '.tt-range-row { display:flex; align-items:center; gap:8px; }',
            '.tt-range-row input[type="range"] { flex:1; }',
            '.tt-range-val { min-width:36px; text-align:center; font-size:11px; color:#38bdf8; font-weight:600; }',
            '.tt-upload-zone { border:2px dashed rgba(100,160,255,0.2); border-radius:12px; padding:24px 16px; text-align:center; cursor:pointer; transition:all .15s; margin-bottom:10px; }',
            '.tt-upload-zone:hover, .tt-upload-zone.dragover { border-color:#38bdf8; background:rgba(56,189,248,.05); }',
            '.tt-upload-zone .tt-icon { font-size:32px; margin-bottom:6px; }',
            '.tt-upload-zone p { font-size:12px; color:#94a3b8; }',
            ...
            '.tt-canvas-hint { position:absolute; top:0; left:0; right:0; padding:4px 10px; background:rgba(0,0,0,0.7); color:#94a3b8; font-size:10px; text-align:center; pointer-events:none; z-index:15; backdrop-filter:blur(4px); }',
            '.tt-overlay-canvas { position:absolute; top:0; left:0; cursor:crosshair; }',
            '.tt-info-bar { position:absolute; bottom:12px; left:50%; transform:translateX(-50%); background:rgba(0,0,0,.7); backdrop-filter:blur(8px); padding:5px 14px; border-radius:22px; font-size:11px; color:#94a3b8; display:flex; gap:14px; z-index:10; }',
            '.tt-info-bar span { display:flex; align-items:center; gap:4px; }',
            '.tt-info-bar .tt-val { color:#38bdf8; font-weight:600; }',
            '.tt-checkbox-row { display:flex; align-items:center; gap:8px; font-size:12px; cursor:pointer; margin-bottom:7px; }',
            '.tt-checkbox-row input { accent-color:#38bdf8; }',
            ...
            '.tt-region-item.selected { background:rgba(56,189,248,.15); border-left-color:#38bdf8; }',
            ...
            '.tt-region-item .tt-info { flex:1; color:#94a3b8; }',
            '.tt-region-item .tt-del { width:18px; height:18px; background:transparent; border:none; color:#94a3b8; cursor:pointer; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:14px; }',
            '.tt-region-item .tt-del:hover { background:#38bdf8; color:#fff; }',
            '.tt-region-item .tt-inner-cb { width:16px; height:16px; accent-color:#00c853; cursor:pointer; flex-shrink:0; }',
            '.tt-toast { position:fixed; top:20px; right:20px; background:rgba(15,25,50,0.95); color:#fff; padding:9px 18px; border-radius:10px; font-size:12px; z-index:99999; transform:translateX(120%); transition:.3s; border-left:3px solid #00c853; pointer-events:none; }',
            '.tt-toast.show { transform:translateX(0); }',
            '.tt-toast.error { border-left-color:#38bdf8; }',
            '.tt-empty-state { text-align:center; color:#94a3b8; }',
            ...
            '.tt-merge-item { position:relative; border-radius:8px; overflow:hidden; background:var(--bg); }',
            ...
            '.tt-merge-item .tt-del-btn { position:absolute; top:2px; right:2px; width:18px; height:18px; background:#38bdf8; color:#fff; border:none; border-radius:50%; font-size:11px; cursor:pointer; display:none; align-items:center; justify-content:center; }',
            '.tt-merge-item:hover .tt-del-btn { display:flex; }',
            '.tt-magnifier { display:none; position:fixed; width:140px; height:140px; border-radius:50%; border:3px solid #38bdf8; box-shadow:0 4px 20px rgba(0,0,0,.6); pointer-events:none; z-index:99998; overflow:hidden; background:#000; }',
            ...
            '.tt-sub-tab{flex:1;padding:5px 8px;border-radius:6px;border:1px solid rgba(100,160,255,0.2);background:rgba(10,18,40,0.9);color:#94a3b8;font-size:12px;cursor:pointer;text-align:center;transition:all .15s;}',
            '.tt-sub-tab:hover{border-color:#38bdf8;color:#38bdf8;}',
            '.tt-sub-tab.active{background:#38bdf8;border-color:#38bdf8;color:#fff;}'
        ].join('\n');
    },
